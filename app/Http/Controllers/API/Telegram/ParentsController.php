...
class ParentsController extends Controller
{
    ...
    {
        $text = "The requested action may not exist in the Parent section yet.";
        $this->data = $data;
        $this->user = $user;
        ...
        $this->panelCommand = config('telegram.commands_button.parents.name');
        $this->panelListCommand = config('telegram.commands_button.parents_list.name');
    ...
    private function userList($commandText = null)
    {
        ...
        $infoArr = [];
        $usersArr = [];
        $totalusers = Parents::count();
        $limit = $this->usersPerView > $totalusers? $totalusers: $this->usersPerView;
        $totaluserslabel = $totalusers > 1? "$totalusers Parents": "$totalusers Parents";
        ...
        $plucks = Parents::pluck('id');
        $users = User::whereIn('id', $plucks)->orderBy('created_at', 'ASC')->offset($activePresent)->limit($limit)->get();
        $countUsers = count($users);
        if($totalusers < $limit) {
            $users = User::whereIn('id', $plucks)->orderBy('created_at', 'ASC')->offset($activePresent)->get();
        }
    ...
    {
        $text = "Details of Guardian (ID: $id)";
